...
Date.prototype.getDateRangeOfWeek = function (weekNo, weekstart) {
  const hostingDateObject = new Date(this); // clone original date
...
Array.prototype.entityPush = function (entity: any, pk?: string, index?: number) {
  ...
  if (typeof entity[pk] === "undefined") {
    return this; // return same array if invalid
  }

  let newArray = [...this]; // clone existing array
  ...
  if (entity[pk] === 0 && entity.tag === 0) {
    ...
    if (typeof index === "undefined" || index === null) {
      ...
      newArray.splice(index, 0, entity);
    }
  } 
...
Array.prototype.entityPop = function (entity, pk?: string) {
  pk = typeof pk === "undefined" ? "id" : pk;

  if (typeof entity[pk] === "undefined") {
    return this;  // Return unchanged array if invalid
  }
  ...
  if (entity[pk] !== 0) {
    if (entity.tag === 4) {
      // Delete After Insert - remove the entity
      newArray = newArray.filter(item => item[pk] !== entity[pk]);
...
Array.prototype.entityReset = function (newObject, pkId?: any) {
  try {
    ...
    if (newObject[pk] === 0 && newObject.tag === 0) {
      // New Insert
      const oldObject = this.find(x => x[pk] === pkId);
      if (oldObject !== null) {
        const newObj = {
          ...newObject,
          [pk]: oldObject[pk],
        ...
        return this.map(item => (item[pk] === pkId ? GlobalMethods.deepClone(newObj) : item));
      }
    } else if (
      newObject[pk] !== 0 &&
      newObject.tag === 0 &&
      ...
      // no change occurred, return original
      return this;
    } else if (
      newObject[pk] !== 0 &&
      newObject.tag === 3 &&
      ...
      // Get from server and modify - restore old object
      const oldObject = this.find(x => x[pk] === pkId);
      if (oldObject !== null) {
        return this.map(item => (item[pk] === pkId ? GlobalMethods.deepClone(oldObject) : item));
      }
    }

    return this; // fallback return original if nothing changed
